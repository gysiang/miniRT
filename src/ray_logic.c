...
/*   Updated: 2024/09/28 21:15:45 by gyong-si         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_ray	make_ray(t_img *data, int x, int y)
{
	...
	v = (1.0 - 2.0 * ((float)y + 0.5) / (float)IMG_HEIGHT) * data->camera.half_height + offset_y;
	//printf("u: %f\n", u);
	//printf("v: %f\n", v);
	...
	ray.vector.z = ray.vector.z > 0 ? 1.0 : ray.vector.z;
...
int trace_ray(t_ray *ray, t_img *data)
{
	...
	printf("Sphere position: (%f, %f, %f), radius: %f\n",
       data->spheres[0].position.x,
       data->spheres[0].position.y,
       data->spheres[0].position.z,
       data->spheres[0].radius);
